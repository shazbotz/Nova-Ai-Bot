...
import { User, Plan, Conversation } from "../memory/models";
...
apiRouter.post("/chat/stream", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const provider = preferredProvider || (user?.preferences as Record<string, unknown>)?.preferredProvider as string | undefined;
    const model = preferredModel || user?.preferences?.preferredModel;
