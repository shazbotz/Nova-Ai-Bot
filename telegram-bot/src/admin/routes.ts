...
// Minimal brute-force throttle for the login form: 5 failed attempts locks an
// IP out for 5 minutes. In-memory only (fine for a single-instance deploy);
// resets on restart.
...
adminRouter.post("/login", (req, res) => {
  ...
  if (isLockedOut(ip)) {
    res.status(429).send(loginHtml("Too many failed attempts. Try again in a few minutes."));
    return;
  }
  ...
  if (
    ...
    safeCompare(username, config.admin.username) &&
