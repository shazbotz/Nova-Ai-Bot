...
export async function setUserPreference(
  ...
  // MongoDB's $set silently drops keys whose value is `undefined` (the driver
  // omits them from the BSON document entirely), so a preference can never be
  // cleared by passing `undefined`. Normalize to `null`, which persists as an
  // explicit "unset" value and round-trips correctly through the schema.
  const normalized = value === undefined ? null : value;
  await User.updateOne(
    { telegramId: userId },
    { $set: { [`preferences.${key}`]: normalized } }
  );
}
