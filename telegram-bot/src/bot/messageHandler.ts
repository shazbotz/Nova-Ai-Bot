...
export function registerMessageHandler(bot: TelegramBot): void {
  bot.on("message", async (msg) => {
    ...
    // ── Guard: deduplicate (prevents double-processing if Telegram redelivers
    // the same update, e.g. due to network retries within a single run)
    ...
    try {
      ...
      if (msg.document || msg.photo || msg.audio || msg.voice) {
        ...
        await handleFileMessage(bot, msg, userId, chatId);
        return;
      }
      ...
      const preferredProvider = user.preferences?.preferredProvider || undefined;
      const preferredModel = user.preferences?.preferredModel || undefined;
      ...
      const isRateLimited = errMsg.includes("ALL_RATE_LIMITED");
...
async function handleFileMessage(
  ...
  // Images and voice notes aren't run through document text-extraction (there's
  // no vision/transcription model wired up), so fail fast with a clear message
  // instead of downloading the file and hitting a confusing "unsupported file
  // type" error later in processFile().
  if (msg.photo || msg.voice || msg.audio) {
    bot.sendMessage(
      chatId,
      msg.photo
        ? "🖼️ Image analysis isn't supported yet — please send text, PDF, DOCX, or code files."
        : "🎙️ Audio/voice transcription isn't supported yet — please type your message or send a text file."
    );
    return;
  }
